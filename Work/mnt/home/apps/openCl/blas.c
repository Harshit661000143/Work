    ...
    for(i = 0; i < n; i++) 
{
 ...
 // Load the kernel source code into the array source_str
  const char *source ="__kernel void saxpy(__global float m,__global float alpha__global float* A, __global float* B, int m,){int x = get_global_id(0);int y = get_global_id(1);if((x<m)&&(y<m)){int value = 0;for(int k = 0; k < m; ++k){int tA = A[k * m + y];int tB = B[k * m+ x];value += tA * tB;}C[y * m + x ] = value;}}";
...
printf("time of execution for size %d is %f [ms]\n",m,time1+time2+time3+time4);
