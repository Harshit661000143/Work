...
#include <CL/cl.h>
#include <math.h>
//#pragma OPENCL EXTENSION __FAST_RELAXED_MATH__ : enable
...
//  const char *source ="__kernel void matrix_add(__global int* A, __global int* B, __global int* C,int m){int x = get_global_id(0);int y = get_global_id(1);if((x<m)&&(y<m)){int value = 0;for(int k = 0; k < m; ++k){int tA = A[y * m + k];int tB = B[k * m+ x];value += tA * tB;}C[y * m + x] = value;}}";
...
//const char *errr="-cl-fast-relaxed-math";
  // Build the program
...
/*
ret= clGetProgramInfo(program,CL_PROGRAM_BINARIES,5000,&binary,NULL);
if(ret!=CL_SUCCESS)
{
printf("\nError binary ");
dump_error(&ret);
...
/*
 FILE *kk;
kk=fopen("binary.txt","w");
fwrite(binary,1,3900, kk);
fclose(kk);
*/
