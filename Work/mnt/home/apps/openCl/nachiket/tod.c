    ...
    for(i = 0; i < n; i++) 
{
 ...
 // Load the kernel source code into the array source_str
  const char *source ="__kernel void dot_pro(__global float* A, __global float* B,__global float* D, __global float* C, int m){int i; C[0]=0.0; int x=get_global_id(0); int num_grp=get_num_groups(0);if(x<m){D[x]=A[x]*B[x];}for(i=0;i<m;i++){C[0]=D[i]+C[0];}}";
   ...
   cl_command_queue command_queue = clCreateCommandQueue( context,device_id,CL_QUEUE_PROFILING_ENABLE, NULL );
    ...
    cl_mem d_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, n * sizeof(float), NULL, &ret);
    cl_mem c_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 1* sizeof(float), NULL, &ret);
...
clGetEventProfilingInfo(event3, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end3, NULL); 
    ...
    ret = clEnqueueReadBuffer(command_queue, c_mem_obj, CL_TRUE, 0,  sizeof(float), C, 0, NULL, &event4);
