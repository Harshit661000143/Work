    ...
    for(i = 0; i < n; i++) 
{
 ...
 // Load the kernel source code into the array source_str
  const char *source ="__kernel void matrix_add(__global int* A, __global int* B, __global int* C,int m){int x = get_global_id(0);int y = get_global_id(1);__local int As[m];__local int Bs[m];if((x<m)&&(y<m)){ int value = 0;for(int k = 0; k < m; ++k){As(y)=A(k*m+y);Bs(x)=B(k*y+x);int ta = As[ y];int tb = Bs[ x];value +=ta  * tb;}C[y * m + x ] = value;}}";
