...
if(m%s!=0)
{
printf("\nERROR!\nnumber of workgroups should be divisible by matrix order\n");
exit(0);
} 
n=m*m;
    int *A = (int*)malloc(sizeof(int)*n);
    int *B = (int*)malloc(sizeof(int)*n);
int *E = (int*)malloc(sizeof(int)*m/s);
    for(i = 0; i < n; i++) 
{
        A[i] = 1;
        B[i] = 1;
if(i<m/s)
 ...
 // Load the kernel source code into the array source_str
  const char *source ="__kernel void matrix_add(__global int* A, __global int* B, __global int* C){int x = get_global_id(0);int y = get_global_id(1);int m=get_local_size(0);int o=get_global_size(0);int n;n=o;int value = 0;for(int k = 0; k < n; ++k){int tA = A[y * n + k];int tB = B[k * n + x];value += tA * tB;}C[y * n + x] = value;}";
