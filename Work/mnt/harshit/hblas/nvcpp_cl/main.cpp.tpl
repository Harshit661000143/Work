...
// *********************************************************************
const char* cSourceFile = "[+NormalName+]_kernels.cl";
...
// *********************************************************************
void [+NameHLower+]Host(const float* pfData1, const float* pfData2, float* pfResult, int iNumElements);
...
int main(int argc, char **argv)
{
    ...
    shrSetLogFileName ("[+NormalName+].txt");
    ...
    // Create the kernel
    ckKernel = clCreateKernel(cpProgram, "[+NameHLower+]", &ciErr1);
    shrLog("clCreateKernel ([+NameHLower+])...\n"); 
    if (ciErr1 != CL_SUCCESS)
    {
    ...
    shrLog("clEnqueueNDRangeKernel ([+NameHLower+])...\n"); 
    if (ciErr1 != CL_SUCCESS)
    {
    ...
    [+NameHLower+]Host ((const float*)srcA, (const float*)srcB, (float*)Golden, iNumElements);
