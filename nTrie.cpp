...
void cluster_anagrams(const vector<string>& word_arr, size_t size)
{
    ...
    for (; it != word_arr.end(); ++it)
    {
      ...
      cout<<"After sort cur_str = "<<cur_str<<endl;
...
int main()
{
    const char* word_arr[] = {"d", "dog", "tac", "god", "act", "gdo", "rand"};
    size_t size = sizeof(word_arr) / sizeof(word_arr[0]);
    cout<<"size ="<<size<<"size of word_arr = "<<sizeof(word_arr)<<"sizeof word_arr[0] = "<<sizeof(word_arr[0])<<endl;
